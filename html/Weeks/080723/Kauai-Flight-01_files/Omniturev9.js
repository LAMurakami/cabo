/****************************************
 //* Numeric Analytics tagging framework Version 0.09
// Last updated June 10th 2008 by Mark Sawlor
****************************************/
var _analytics_ = new Object;

// ******************   CONFIG *********************************
_analytics_.config=new Object;
_analytics_.config.baseUrl="//www.alaskaair.com/javascripts/analytics/";
_analytics_.config.s_account="alaskadev";
_analytics_.config.s_visitorNamespace="alaskaairlines";
_analytics_.config.s_trackingServer="omni.alaskaair.com";
_analytics_.config.s_trackingServerSecure="somni.alaskaair.com";
_analytics_.config.s_linkInternalFilters="javascript:,alaskaair.com"
if(! _analytics_.config.pnPrefix) _analytics_.config.pnPrefix="";
// usesJS_PageName_Override is used to toggle the Javascript PageName generator which looks to page h1 tags to assemble the PageName.
_analytics_.config.usesJS_PageName_Override = true; // if changed to false the only PageName manipulation preformed will be toLowercase()

_analytics_.config.openScrpt = '<scr' + 'ipt language="JavaScript" type="text/javascript" src="';
_analytics_.config.closeScrpt = '"></scr' + 'ipt>';
_analytics_.s = new Object;
// ******************   CONFIG *********************************

// ******************   UTILS *********************************
_analytics_.utils=new Object;
_analytics_.utils.loadScriptLibrary = function(src, targetElement) { 
	try{
		if (_analytics_.scriptElement == null) {
			_analytics_.scriptElement = document.createElement('script');
			_analytics_.scriptElement.type = 'text/javascript'; 
		}
		if (targetElement == null){
			if (_analytics_.headElement == null)
				_analytics_.headElement = document.getElementsByTagName('body')[0];
			targetElement = _analytics_.headElement;
		}
		var library = _analytics_.scriptElement.cloneNode(true);
		library.src = src;
		targetElement.appendChild(library);
	}catch(e){}
}

_analytics_.utils.addOnloadEvent = function(func){
		try{
...
_analytics_.utils.queryStringToHash = function(str){
		try{
			var qs=[];
...
_analytics_.utils.readCookie = function(name){
		try{
			var name=name+"=";
...
_analytics_.utils.setCookie = function(name,value){
		try{
			document.cookie=name+"="+value+"; path=/";
		}catch(e){}
	}
_analytics_.utils.decode = function( $str ){
	try{
...
_analytics_.utils.getText = function(nodeOrig){
	try{
	 var rtn= "";
	 var t=0;
	 	...
	 	if(t < 100){
		 	for(var c=0;c<node.childNodes.length;c++){
				...
				switch(childnode.nodeType){
					case 3: 
						rtn += _analytics_.utils.decode(childnode.nodeValue);
						break;
					case 1:
						t++;
						fcn(childnode);
...
_analytics_.isIE = (window.navigator.userAgent.indexOf("MSIE") > 0);
if (!  _analytics_.isIE ) {
...
//Omniture
//var s = new Object;
var s_account=_analytics_.config.s_account;
var s=new Object;
...
function s_doPlugins(s) {
	...
	if(!s.campaign)
		s.campaign=s.getQueryParam('cid,wc_mid',':');
	/*Please replace 'cid' with the querystring parameter used for tracking external campaigns*/	
		s.campaign=s.getValOnce(s.campaign,"cname",0);
		
		s.prop23=s.getAndPersistValue(s.campaign,'s_cp_persist',0);

		
		s.prop20=s.getTimeParting('h','-8','2008'); // Set hour 
		s.prop19=s.getTimeParting('d','-8','2008'); // Set day
		s.prop21=s.getTimeParting('w','-8','2008'); // Set Weekend / Weekday
		
		if(s.prop20)
		s.eVar26=s.prop21 + " : " + s.prop19 + " : " + s.prop20;
		
		s.prop24=s.eVar27=s.getNewRepeat();
		s.eVar18=s.getQueryParam('incid');
		s.setupFormAnalysis();				
		s.eVar49=s.getQueryParam("wc_rid"); //Genesis WhatCounts VisitorID
}

/****************************** Plugin Config **********************************/

_analytics_.analyticsCallBack = function() {
	 				s_account=_analytics_.config.s_account;
	 				_analytics_.s = s;
					s=s_gi(s_account);
					for(i in _analytics_.s){
						if(_analytics_.s[i] != ""){s[i]= _analytics_.s[i];};
					}
					/************************** PLUGINS SECTION *************************/
...
/************************** PLUGINS SECTION *************************/
					_analytics_.omni = new Object;
					//used to clear all existing s.prop values
					_analytics_.omni.clearProps = function(){
						try{
							for(var p=1;p<=50;p++) {s[("prop"+p)]="";}
						}catch(e){}
					}
					//used to clear all existing s.eVar values
					_analytics_.omni.clearVars = function(){
						try{
							for(var p=1;p<=50;p++) {s[("eVar"+p)]="";}
						}catch(e){}
					}
					/***************************** Page Var logic here ************************************/
				s_account=_analytics_.config.s_account;
				s.visitorNamespace=_analytics_.config.s_visitorNamespace;
				s.trackingServer=_analytics_.config.s_trackingServer;
				s.trackingServerSecure=_analytics_.config.s_trackingServerSecure;
				s.dc=112;
				_analytics_.page = new Object;
				_analytics_.page.h1 = document.getElementsByTagName("h1")[0];
				if(typeof _analytics_.page.h1  != "undefined"){_analytics_.page.h1.text = _analytics_.utils.getText(_analytics_.page.h1);};
				if(_analytics_.config.usesJS_PageName_Override){
					// if there is no pre defined PageName create one
					if(_analytics_.config.pnPrefix != "") {
						_analytics_.config.pnParts = 3;
					}else{
						_analytics_.config.pnParts = 2;
					}
					
					if(s.pageName == "" || typeof s.pageName == "undefined" ){
						s.pageName = ""
						if(_analytics_.config.pnPrefix != ""){s.pageName = _analytics_.config.pnPrefix + " : ";};
						if(s.channel !="" && typeof s.channel != "undefined"){s.pageName += s.channel + " : "}; 
						if(typeof _analytics_.page.h1  != "undefined"){_analytics_.page.h1.text == "" ? s.pageName += _analytics_.page.h1.innerText : s.pageName += _analytics_.page.h1.text};
					// are there multiple parts of the PageName present
					}else if(s.pageName.indexOf(':') != -1){
						_analytics_.page.nameArr = s.pageName.split(':');
						// if all parts of the PageName are present 
						if(_analytics_.page.nameArr.length == _analytics_.config.pnParts){
							if(_analytics_.page.nameArr[(_analytics_.config.pnParts -1)].indexOf('.') != -1){
								if(typeof _analytics_.page.h1  != "undefined"){
									_analytics_.page.h1.text == "" ? _analytics_.page.nameArr[(_analytics_.config.pnParts -1)] = _analytics_.page.h1.innerText : _analytics_.page.nameArr[(_analytics_.config.pnParts -1)] = _analytics_.page.h1.text;
									s.pageName = _analytics_.page.nameArr.join(" : ");
								}
							}
						// if parts of the PageName are missing
						}else if(_analytics_.page.nameArr.length < _analytics_.config.pnParts){
								if(_analytics_.page.nameArr[(_analytics_.page.nameArr.length -1)].indexOf('.') != -1){
									if(typeof _analytics_.page.h1  != "undefined"){
										_analytics_.page.h1.text == "" ? _analytics_.page.nameArr[(_analytics_.config.pnParts -1)] = _analytics_.page.h1.innerText : _analytics_.page.nameArr[(_analytics_.config.pnParts -1)] = _analytics_.page.h1.text;
									}
								}else{
									_analytics_.page.nameArr[(_analytics_.config.pnParts -1)] =_analytics_.page.nameArr[(_analytics_.page.nameArr.length -1)];
								}
							if(s.channel !="" && typeof s.channel != "undefined"){_analytics_.page.nameArr[(_analytics_.config.pnParts -2)]  = s.channel;}; 
							}
							if(_analytics_.config.pnPrefix != "") {
								_analytics_.page.nameArr[0] = _analytics_.config.pnPrefix;
							}
							s.pageName = _analytics_.page.nameArr.join(" : ");
					//	only the last portion of the PageName is present
					}else{	
						if(s.channel !="" && typeof s.channel != "undefined"){s.pageName = s.channel + " : " +s.pageName;}; 
						if(_analytics_.config.pnPrefix != "") {s.pageName = _analytics_.config.pnPrefix +" : " + s.pageName;};
					}
				}
				s.pageName =s.pageName.toLowerCase();
					...
					s.linkInternalFilters=_analytics_.config.s_linkInternalFilters;
					...
					///////////////////////////// Numeric Analytics Custom Search Marketing Code v 1.1 /////////////////////////// 
					//::::::::::::::::::::::::::::::::::::::::::: CONFIGURATION :::::::::::::::::::::::::::::::::::::::::::::::
					
					var ppcdetection="kwcid"; //Define the query string parameter that is only found in your PPC urls and not your natural urls. This will likely be the same setting as found in the Omniture > Admin > Report Suite Manager > Edit Settings > General > Paid Search Detection
					
					//You will need to set up 4 custom evars within the SiteCatalyst Admin area and specify the evar numbers below:
						// a.) Natural Keyword (1st in 30 days) - Set allocation to "Original Value (First)", Expire after to "Month" and Status to "Basic Subrelations"
						// b.) Natural Keyword (Last in 30 days) - Set allocation to "Most Recent (Last)", Expire after to "Month" and Status to "Basic Subrelations"
						// c.) Paid Keyword (1st in 30 days) - Set allocation to "Original Value (First)", Expire after to "Month" and Status to "Basic Subrelations"
						// d.) Paid Keyword (Last in 30 days) - Set allocation to "Most Recent (Last)", Expire after to "Month" and Status to "Basic Subrelations"
					var firstnatkw="38"; //Specify the evar number for "Natural Keyword (1st in 30 days)"
					var lastnatkw="39"; //Specify the evar number for "Natural Keyword (Last in 30 days)"
					var firstpaidkw="40"; //Specify the evar number for "Paid Keyword (1st in 30 days)"
					var lastpaidkw="41"; //Specify the evar number for "Paid Keyword (Last in 30 days)"
					
					//::::::::::::::::::::::::::::::::::::::::: END CONFIGURATION :::::::::::::::::::::::::::::::::::::::::::::
					
					var rurl=document.referrer; //get the referring url
					var curl=location.href; //get the current url
					
					//************************** BEGIN 1st & LAST NATURAL SEARCH DETECTION **************************************/					
					//Set the referring search engine variable and parameter name used for keyword depending on the value of "rurl"
					if(rurl.indexOf("google.com/search") != -1) { var searchengine="Google"; var keywordvar="q="; }
					else if (rurl.indexOf("search.live.com") != -1) { var searchengine="MS Live"; var keywordvar="q="; }
					else if (rurl.indexOf("search.msn.com") != -1) { var searchengine="MSN"; var keywordvar="q="; }
					else if (rurl.indexOf("www.ask.com") != -1) { var searchengine="ASK"; var keywordvar="q="; }
					else if (rurl.indexOf("search.aol.com") != -1) { var searchengine="AOL Search"; var keywordvar="query="; }
					else if (rurl.indexOf("search.yahoo.com") != -1) { var searchengine="Yahoo!"; var keywordvar="p="; }
					else { var searchengine=""; }				
					//if the searchengine variable has a value and the ppcdetection does NOT
					if((searchengine != "") & (curl.indexOf(ppcdetection)) == -1)
					   {
						   //create "slicekw" variable equal to portion of query string starting with keyword variable
						   slicekw=rurl.slice(rurl.indexOf(keywordvar));
						   //replace all +'s with spaces so SiteCatalyst reports look nicer
						   slicekw=slicekw.replace(/\+/g, " ");
						   //check to see if there are any additional query parameters besides keyword variable (look for "&")
						   if (slicekw.indexOf("&") != -1)
						   		{ //if "&" is found then set evar to characters after "q=" or "p=" but before the first "&"
									s["eVar"+firstnatkw]="[" + searchengine + "] - " + (slicekw.substr(keywordvar.length,(slicekw.indexOf("&"))-2));
								}
						   else //otherwise keyword parameter is the only parameter so set evar equal to the number of characters after q or p.
						   		{
									s["eVar"+firstnatkw]="[" + searchengine + "] - " + (slicekw.substr(keywordvar.length,slicekw.length));
								}
						   s["eVar"+lastnatkw]=s["eVar"+firstnatkw]; //set "last" keyword evar equal to 1st (Omniture takes care of attribution in reports)
					   }
					//************************** END 1st & LAST NATURAL SEARCH DETECTION **************************************/
					
					//************************** BEGIN 1st & LAST PAID SEARCH DETECTION **************************************/
					
					if(curl.indexOf("gkey=") != -1) { s["eVar"+firstpaidkw]="[Google] - " + s.getQueryParam('gkey'); }
					else if(curl.indexOf("OVKEY=") != -1) { s["eVar"+firstpaidkw]="[Yahoo] - " + s.getQueryParam('OVKEY'); }
					else if(curl.indexOf("mskey=") != -1) { s["eVar"+firstpaidkw]="[MSN] - " + s.getQueryParam('mskey'); }
					else {s["eVar"+firstpaidkw]=""; }
					
					s["eVar"+lastpaidkw]=s["eVar"+firstpaidkw];
					
					//************************** END 1st & LAST PAID SEARCH DETECTION **************************************/		
					var s_code=s.t();
					//if(s_code){_analytics_.bodyElement= document.getElementsByTagName('body')[0];_analytics_.bodyElement.innerHTML +=s_code;}	
