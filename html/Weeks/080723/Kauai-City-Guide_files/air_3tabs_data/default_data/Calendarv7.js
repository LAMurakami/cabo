var maxValidDate = '';
var minValidDate = '';
function OpenCalendar(oElement,xOffset) 
{ 
	var sName = oElement.name; 
	var sValue = oElement.value; 
	var iPopupWidth = 240; 
	var iPopupHeight = 240; 
	var iTop = 300; 
	var iLeft = 300; 
	if(navigator.userAgent.indexOf('Netscape/8')>0 && navigator.userAgent.indexOf('MSIE')>0)
	{ 
		iPopupWidth = 247; 
		iPopupHeight = 228;
	}
	if(window.event) 
	{ 
		iTop = event.screenY - (iPopupHeight/3); 
		iLeft = event.screenX + xOffset;
	}
	var popup = window.open('/calendar/default.aspx?target=' + sName + '&value=' + sValue + '&maxValidDate=' + maxValidDate + '&minValidDate=' + minValidDate,'Calendar','width='+iPopupWidth+',height='+iPopupHeight+',resizable=no,scrollbars=no,titlebar=no,dependent=yes,alwaysRaised=yes,left=' + iLeft + ',top=' + iTop); 
	popup.focus();
}
function OpenCalendar3(sControlID,xOffset, maxDate)
{ 
	maxValidDate = maxDate; 
	OpenCalendar2(sControlID,xOffset); 
}
function OpenCalendar2(sControlID,xOffset)
{ 
	var oElement = document.forms[0][sControlID]; 
	OpenCalendar(oElement,xOffset); 
}
function OpenCalendar4(sControlID, xOffset, maxDate, minDate)
{ 
	maxValidDate = maxDate; 
...
return null;}
function OpenCalendar5(sControlID, sControlID2, xOffset) {
    var sControlIDItems = sControlID.split('|');
    var oElement;
    if(sControlIDItems != null && sControlIDItems.length == 2) {
        oElement = document.forms[0][sControlIDItems[1]];        
    } else {
        oElement = document.forms[0][sControlID];        
    }   
    var sControlID2Items = sControlID2.split('|');
    var oElement2;
    if(sControlID2Items != null && sControlID2Items.length == 2) {
        oElement2 = document.forms[0][sControlID2Items[1]];
    } else {
        oElement2 = document.forms[0][sControlID2];
    }    
    var sName = sControlIDItems[0] + '|' + oElement.name;    
    var sName2 = sControlID2Items[0] + '|' + oElement2.name;
    var sValue = oElement.value;    
    var iPopupWidth = 240;
    var iPopupHeight = 240;
    var iTop = 300;
    var iLeft = 300;
    if(navigator.userAgent.indexOf('Netscape/8')>0 && navigator.userAgent.indexOf('MSIE')>0){ 
        iPopupWidth = 247; iPopupHeight = 228;
    }
    if(window.event) { 
        iTop = event.screenY - (iPopupHeight/3); iLeft = event.screenX + xOffset;
    }
    var popup = window.open('/calendar/default.aspx?target=' + sName +  '|' + sName2 + '&value=' + sValue + '&maxValidDate=' + maxValidDate,'Calendar','width='+iPopupWidth+',height='+iPopupHeight+',resizable=no,scrollbars=no,titlebar=no,dependent=yes,alwaysRaised=yes,left=' + iLeft + ',top=' + iTop); 
    popup.focus();
}
