...
s.linkInternalFilters="javascript:,alaskaair.com"
s.linkLeaveQueryString=true
s.linkTrackVars="None"
s.linkTrackEvents="None"
s.trackingServer="omni.alaskaair.com";
s.trackingServerSecure="somni.alaskaair.com";
...
function s_doPlugins(s) {
	...
	// MASK EMAIL
	if(s.eVar25)
		s.eVar25=s.maskEmail(s.eVar25);	
	if(s.eVar29)
		s.eVar29=s.maskEmail(s.eVar29);
	
	// FOR PAGENAME
	getText = function(nodeOrig){try{var rtn= "";var t=0;var fcn = function(node){if(t < 100){for(var c=0;c<node.childNodes.length;c++){var childnode=node.childNodes[c];switch(childnode.nodeType){case 3:rtn += decode(childnode.nodeValue);break;case 1:t++;fcn(childnode);break;};}}};fcn(nodeOrig);return rtn;}catch(e){return "";}}
	decode = function( $str ){try{var $entities = [ ['%f8','%20'], ['%a0', '%20'],  ['%3e','%20'], ['%3c','%20'],  ['%26','%20'],  ['%22','%20'] ];var $ii = 0;var $estr = escape( $str );$estr = $estr.toLowerCase();while ($ii < $entities.length){$estr = $estr.replace(new RegExp($entities[$ii][0], "g"), $entities[$ii][1] );$ii++;}return unescape($estr);}catch(e){return $str;}}
	
	// INTERNAL MARKETING
	var qp = "", qp2 = "";
	if(!s.campaign)
	{
		qp=s.getQueryParam('int');
		if(qp)
			s.campaign="INT-_"+qp;
	}
	...
	if(!s.campaign)
	{
		qp=s.getQueryParam('wc_mid');
		qp2=s.getQueryParam('wc_lid');
		if(qp)
			s.campaign=qp+":"+qp2;
	}
	if(!s.campaign)
	{
		qp=s.getQueryParam('semid');
		if(qp)
			s.campaign="SEM-_"+qp;
	}
	if(!s.campaign)
	{
		qp=s.getQueryParam('s_kwcid');
		if(qp)
			s.campaign="SEM-_"+qp;
	}
	if(!s.campaign)
	{
		qp=s.getQueryParam('eml');
		if(qp)
			s.campaign="EML-_"+qp;
	}
	if(!s.campaign)
	{
		qp=s.getQueryParam('ola');
		if(qp)
			s.campaign="OLA-_"+qp;
	}
	if(!s.campaign)
	{
		qp=s.getQueryParam('soc');
		if(qp)
			s.campaign="SOC-_"+qp;
	}
	if(!s.campaign)
	{
		qp=s.getQueryParam('meta');
		if(qp)
			s.campaign="META-_"+qp;
	}
	if(!s.campaign)
	{
		qp=s.getQueryParam('afl');
		if(qp)
			s.campaign="AFL-_"+qp;
	}
	s.campaign=s.getValOnce(s.campaign,"cname",0);
	
	// DO NOT SET EXT CAMPAIGN VARIABLES WITH INT TRACKING CODES:
	var re=new RegExp(/^AS_/);
	if(!re.test(s.campaign))
	{
		var re2=new RegExp(/^EB_/);
		if(!re2.test(s.campaign))
		{
			s.eVar38=s.eVar39=s.eVar40=s.campaign;
		}
		else s.eVar19=s.eVar20=s.campaign;
	}
	else s.eVar19=s.eVar20=s.campaign;
	...
	bracketRegExp = new RegExp('\\[[a-zA-Z]{2}\]', 'gi');  // FINDS [CS] & [CE]
	prefixRegExp = new RegExp('[a-zA-Z]{1}[0-9]{1}\\|', 'gi');  // FINDS v1|

	visitorid =  s.c_r('s_vi');

	if(visitorid != null && visitorid.length > 0)
	{
		visitorid = visitorid.replace(bracketRegExp, '');
		if(visitorid != null && visitorid.length > 0)
		{
			visitorid = visitorid.replace(prefixRegExp, '');
		}
	}
	...
	// GENESIS WHATCOUNTS INTEGRATION
	if(!s.eVar18)
		s.eVar18=s.getQueryParam('offid'); // OFFER ID
	s.eVar18=s.getValOnce(s.eVar18,"s_eVar18",0); 
	s.eVar49=s.getQueryParam("wc_rid"); // VISITOR ID
	if(!s.eVar15)
		s.eVar15=s.getQueryParam("wc_lid"); // LINK ID
	...
	// PAGENAME
	var usesJS_PageName_Override = true; // IF FALSE, ONLY PAGENAME MANIPULATION WILL BE TOLOWERCASE
	var pnParts, pnPrefix = "";
	page = new Object;
	page.h1 = document.getElementsByTagName("h1")[0];
	if(typeof page.h1  != "undefined")
		page.h1.text = getText(page.h1);
	if(usesJS_PageName_Override)
	{
		// IF NO PRE-DEFINED PAGENAME, CREATE ONE
		if(pnPrefix != "")
			pnParts = 3;
		else
			pnParts = 2;
		...
		if(s.pageName == "" || typeof s.pageName == "undefined" )
		{
			...
			if(pnPrefix != "")
				s.pageName = pnPrefix + " : ";
			if(s.channel !="" && typeof s.channel != "undefined")
				s.pageName += s.channel + " : ";
			if(typeof page.h1  != "undefined")
				page.h1.text == "" ? s.pageName += page.h1.innerText : s.pageName += page.h1.text;
		}
		// ARE THERE MULT PARTS OF PAGENAME PRESENT
		else if(s.pageName.indexOf(':') != -1)
		{
			page.nameArr = s.pageName.split(':');
			// IF ALL PARTS OF PAGENAME ARE PRESENT
			if(page.nameArr.length == pnParts)
			{
				if(page.nameArr[(pnParts -1)].indexOf('.') != -1)
				{
					if(typeof page.h1  != "undefined")
					{
						...
						s.pageName = page.nameArr.join(" : ");
					}
				}
			}
			// IF PARTS OF PAGENAME ARE MISSING
			else if(page.nameArr.length < pnParts)
			{
				...
				if(s.channel !="" && typeof s.channel != "undefined")
					page.nameArr[(pnParts -2)]  = s.channel;
			}
			if(pnPrefix != "")
				page.nameArr[0] = pnPrefix;
			s.pageName = page.nameArr.join(" : ");
		}
		// ONLY THE LAST PORTION OF THE PAGENAME PRESENT
		else
		{
			if(s.channel !="" && typeof s.channel != "undefined")
				s.pageName = s.channel + " : " +s.pageName;
			if(pnPrefix != "")
				s.pageName = pnPrefix +" : " + s.pageName;
		}
	}
	s.pageName =s.pageName.toLowerCase();
	s.eVar48=s.pageName; // ENTRY PAGE
	
	// LANDING PV AND CLICK PAST
	s.setLPVandClickPast('event21','event22');
	
	// TARGETING COOKIES
	var expire=new Date(),ct=expire.getTime();
	expire.setTime(ct+31536000000);
		
	if(s.eVar22) // MEMBERSHIP LEVEL
	{
		s.c_w("AS_mbr",s.eVar22,expire);
	}
	
	if(s.eVar2) // DESTINATION
	{
		s.c_w("AS_dest",s.eVar2,expire);
	}
	
	if(s.eVar9) // FAMILY
	{
		s.c_w("AS_fam",s.eVar9,expire);
	}
	
	if(s.eVar34) // CARD HOLDER
	{
		if(s.eVar34.indexOf('BAAS') != -1)
			s.c_w("AS_card",'BAAS',expire);
	}
	
	var url=s.linkHandler('bluestreak.com|applyonlinenow.com|mbna.com','o'); // BANK CARD LINKS
	if(url){
		s.prop16 = s.pageName + " | Bank Card Link";
		s.events = "event23";
...
s.visitorNamespace="alaskaairlines"
s.dc="112"
...
/*
* Added by Bhanu Mullapudi on 08/05/2009 to enable site analysis on Test and Target site.
...
/*
 * Plugin: Mask Email v.1.0
 */
s.maskEmail=new Function("addr",""
+"var s=this;if(addr.substring(0,3) != '^v^'){var str='';addr = addr."
+"toLowerCase();for(var i=0;i<addr.length;i++){str+=s.encode(addr.subst"
+"ring(i,i+1));}return '^v^'+str;}else return addr;");
s.encode=new Function("char",""
